...
$\times$ 12 lần tích lũy; kích thước micro-batch 40 được chọn bằng đo đạc
thực nghiệm để lấp 91\% VRAM (mục~\ref{sec:exp-gpu}). 6.000 bước tương ứng
$\approx$2,95 tỷ token ($\approx$0,98 epoch --- mỗi mẫu chỉ được thấy xấp xỉ
một lần). Toàn bộ chạy \texttt{bfloat16} (autocast) với
\texttt{torch.compile} \autocite{ansel2024pytorch2}.
...
Vì phiên huấn luyện kéo dài nhiều giờ trên hạ tầng từ xa, tính
\textbf{khôi phục chính xác} được thiết kế và kiểm thử tường minh: checkpoint
lưu đủ trọng số, trạng thái optimizer, bước hiện tại và \emph{mọi trạng thái
sinh số ngẫu nhiên} (bộ sinh lấy mẫu dữ liệu, RNG của PyTorch trên CPU lẫn
GPU). Unit test khóa bất biến ``huấn luyện 6 bước liền mạch $=$ huấn luyện 3
bước + khôi phục + 3 bước'' chính xác đến từng bit trọng số. Một lỗi thực tế
đáng ghi nhận: tensor trạng thái RNG bị \texttt{map\_location} đưa lên GPU khi
nạp checkpoint, trong khi API khôi phục đòi hỏi tensor CPU --- lỗi chỉ phát lộ
trên máy GPU, được vá và khóa bằng test riêng.
...
\section{Hạ tầng, kiểm thử và đo đạc}
\label{sec:met-infra}

Toàn bộ huấn luyện chạy trên một máy trạm từ xa: GPU NVIDIA RTX PRO 4000
Blackwell 24\,GB, 24 lõi CPU, 31\,GB RAM, môi trường WSL2 (Ubuntu trên
Windows), PyTorch 2.11/CUDA 12.8; phiên dài chạy trong \texttt{tmux} để sống
sót qua đứt kết nối SSH. Phát triển và kiểm thử trên máy cá nhân không GPU:
\textbf{52 unit test} chạy ngoại tuyến (mô hình thu nhỏ, bộ mã hóa thị giác
giả lập, dữ liệu tự sinh) khóa các bất biến quan trọng --- RoPE bảo toàn
chuẩn, mặt nạ nhân quả, đếm đúng 100.682.496 tham số, trói trọng số, che nhãn
SFT đúng vị trí, khôi phục checkpoint chính xác từng bit, căn chỉnh
log-xác suất SCST qua 49 token ảnh, và một test hồi quy cho lỗi đường dẫn ảnh
phát hiện khi rà soát chéo. Suốt quá trình huấn luyện, một tiến trình phụ ghi
mức sử dụng GPU và VRAM mỗi 30 giây (\texttt{nvidia-smi}) --- nguồn số liệu
cho phân tích hiệu suất ở mục~\ref{sec:exp-gpu}. Khi đánh giá sinh chuỗi bằng
