...
\begin{enumerate}
	\item Tiền huấn luyện từ đầu một mô hình 101M tham số kiến trúc Llama-style
	      trên khoảng 3 tỷ token tiếng Việt, bằng đúng một GPU 24\,GB, đạt được
	      chất lượng ngôn ngữ đến đâu (đường cong loss, perplexity,
	      bits-per-character so với baseline GPT-2 tiếng Việt huấn luyện sẵn)?
...
Ngoài ra, đề tài có một mục tiêu hạ tầng tường minh: \emph{sử dụng trọn} tài
nguyên của máy huấn luyện (GPU 24\,GB) --- cả về mức chiếm dụng tính toán lẫn
bộ nhớ --- trong khoảng một ngày GPU, với nhật ký \texttt{nvidia-smi} ghi mỗi
30 giây làm bằng chứng định lượng (mục~\ref{sec:exp-gpu}).
...
Việt từ dữ liệu web thô đến VLM biết trả lời bằng tiếng Việt trên một GPU;
(ii) các đo đạc định lượng về chất lượng ngôn ngữ, hiệu quả tokenizer, chất
lượng đa phương thức và hiệu suất phần cứng ở quy mô ``nano''; (iii) một bộ
bài học hạ tầng thực tế (huấn luyện dài ngày trên WSL2, khôi phục checkpoint
chính xác từng bit, \texttt{torch.compile} trên kiến trúc GPU mới).
...
lượng và định tính kèm phân tích hiệu suất phần cứng.
